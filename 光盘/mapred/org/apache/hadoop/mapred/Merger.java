...
class Merger {  
  ...
  public static class Segment<K extends Object, V extends Object> {
    ...
    public Segment copy(Configuration conf) throws IOException, InterruptedException{
//        LOG.info("Segment:copy");
        return new Segment(this.reader.copy(conf),
                this.key.copy(),
                this.value.copy(),
                this.segmentOffset,
                this.segmentLength);
    ...
    public Segment(Reader<K, V> reader,
                    ...
                    long segmentOffset,
                    long segmentLength) throws IOException {
      this.reader = reader;
      this.key = key;
      this.value = value;
  ...
  extends PriorityQueue<Segment<K, V>> implements RawKeyValueIterator {
    ...
    @Override
    public RawKeyValueIterator copy(Configuration conf) throws IOException, InterruptedException {
//        LOG.info("mergequeue:copy start");
//        List<Segment<K, V>> copysegments = new ArrayList<Segment<K,V>>();
//
//        for(Segment<K, V> s : this.segments){
//                    copysegments.add(s.copy(conf));   
//        }
//        
//        MergeQueue mq =  new MergeQueue(null,this.comparator,null,
//                this.key.copy(),this.value.copy());
        MergeQueue mq;
        if(this.key == null){
            mq =  new MergeQueue(null,this.comparator,null,null,null);
        }
        mq =  new MergeQueue(null,this.comparator,null,
                this.key.copy(),this.value.copy());
        super.copy(mq);
        
        //mq.heap = new Object[this.heap.length];
        for(int i = 0; i < this.getheaplength(); i++){
//            LOG.info("this.heap.length:"+this.getheaplength()+":" + i);
            Segment ori = this.getheap(i);
            if(ori == null ){
...
//                if(ori == null)
//                    LOG.info("ori == null");
                Segment seg = ori.copy(conf);
                mq.copyput(seg, i);
                if(this.minSegment == ori){
                    mq.minSegment = seg;
                }
            }
        }
...
//        LOG.info("mergequeue copy over");
        return mq;

        
    }
    
    public MergeQueue(List<Segment<K, V>> segments, RawComparator<K> comparator,
        Segment<K, V> minSegment,DataInputBuffer key,DataInputBuffer value) {
...
//      LOG.info("adjustPriorityQueue hasnext:"+ hasNext);
    ...
    public boolean next() throws IOException {
      if (size() == 0){
//          LOG.info("1 next size = " + size());
        return false;
      }
//      LOG.info("2 next size = " + size());
      if (minSegment != null) {
//          LOG.info("minSegment != null");
        //minSegment is non-null for all invocations of next except the first
        //one. For the first invocation, the priority queue is ready for use
        //but for the subsequent invocations, first adjust the queue 
        adjustPriorityQueue(minSegment);
        if (size() == 0) {
//            LOG.info("3 next size = " + size());
          minSegment = null;
          return false;
        }
      }
//      LOG.info("4 next size = " + size());
      minSegment = top();
