  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    private RawKeyValueIterator createKVIterator(
      ...
      if (0 != onDiskBytes) {
        ...
        finalSegments.add(new Segment<K,V>(
              new RawKVIteratorReader(diskMerge, onDiskBytes, conf), true));
      }
    ...
    class RawKVIteratorReader extends IFile.Reader<K,V> {
      ...
      public Reader copy(Configuration conf) throws IOException, InterruptedException{
          LOG.info("RawKVIteratorReader:copy");
          Reader r = super.copy(conf);
          RawKVIteratorReader copy = null;

          copy = new RawKVIteratorReader(kvIter.copy(conf), r, conf);

          return copy;
      }
      
      public RawKVIteratorReader(RawKeyValueIterator kvIter, Reader r, Configuration conf)
          throws IOException {
        super(r.in,
                r.bytesRead,
                r.fileLength,
      ...
      public RawKVIteratorReader(RawKeyValueIterator kvIter, long size, Configuration conf)
          throws IOException {
        super(conf, null, size, null, spilledRecordsCounter);
        this.kvIter = kvIter;
      }
