...
class IFileInputStream extends InputStream {
  ...
  private static final org.apache.commons.logging.Log LOG = LogFactory.getLog(IFileInputStream.class);
  
  public IFileInputStream copy(Configuration conf){
//      if(sum == null)
//          LOG.info("IFileInputStream copy:sum == null");
//      LOG.info("nio:"+conf.getBoolean("nio", false));
        if(csum == null){
            return new IFileInputStream(in,
              length,
              dataLength,
              sum.copy(),
              currentOffset,
          ...
          //LOG.info("IFileInputStream copy:currentOffset: " + currentOffset);
      return new IFileInputStream(in,
              length,
              dataLength,
              sum.copy(),
...
//      LOG.info("copy:currentOffset:"+currentOffset);
      //this.b = b;
      this.csum = csum;
...
//      if(conf == null)
//          LOG.info("IFileInputStream construct:conf == null");
      this.conf = conf;
    this.in = in;
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    if (currentOffset >= dataLength) {
//        LOG.info("this:ifileinputstream:"+this.toString());
//        LOG.info("currentOffset:"+currentOffset+" dataLength:"+dataLength);
      return -1;
    }
...
//        LOG.info("conf != null");
//        LOG.info("nio == "+conf.getBoolean("nio", false));
//        if(conf.getBoolean("nio", false)){
//            
//            if(in instanceof FSDataInputStream){
//                LOG.info("in instanceof FSDataInputStream");
//            }
//        }   
//    }


    if(conf != null && conf.getBoolean("nio", false) && in instanceof FSDataInputStream){
        bytesRead = ((FSDataInputStream)in).read(currentOffset, b, off, len);
//        position += bytesRead;
//        LOG.info("nio == ture position " + position);
    }else{    
...
//    LOG.info("currentOffset " + currentOffset);
    ...
    if (currentOffset == dataLength) {
      ...
      if(conf == null || !conf.getBoolean("nio", false)){
        csum = new byte[checksumSize];
        IOUtils.readFully(in, csum, 0, checksumSize);
        if (!sum.compare(csum, 0)) {
          throw new ChecksumException("Checksum Error", 0);
  //          Log.info("Checksum Error " + csum.toString() + " " + sum.toString());
        }
      }
    }
    return bytesRead;
